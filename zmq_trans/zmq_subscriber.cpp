...
void ZmqStructSend::AddStructData(SaveBufferIndex index_data, const std::string &data) {
    ...
    if (!mStop) {
        ...
        memcpy(data_ptr, &(it->second), sizeof(int32_t));
        memcpy(data_ptr + 4, &index_data.topic_id, sizeof(int32_t));
        memcpy(data_ptr + 4 + 4, &index_data, sizeof(index_data));
        memcpy(data_ptr + 4 + 4 + sizeof(index_data), data.c_str(), data.size());
